...
"""
from core.exotel_client import ExotelClient
            ...
            async for message in websocket:
                try:
                    ...
                    if stream_id not in self.exotel_connections:
                        ...
                        # Create Call Session
                        session = self.exotel_client.create_session(
                            stream_id=stream_id,
                            websocket=websocket
                        )
                        session.sample_rate = self.connection_sample_rates.get(
                            stream_id,
                            self.default_sample_rate
                        )
                        # Start Conversation

                        greeting = self.conversation_manager.start(stream_id)
                        logger.info(f"🤖 {greeting}")
                        question = self.conversation_manager.next_question(stream_id)
                        if question:
                            key, text = question
            ...
            async for message in Gemini_ws:
                try:
                    ...
                    if event_type == "response.audio.delta":
                        ...
                        session = self.exotel_client.get_session(stream_id)
                        if session:
                            question = self.conversation_manager.next_question(stream_id)
                            if question:
                                field, text = question
                                logger.info(f"🎤 Next Question: {text}")
                            else:
                                logger.info("✅ Interview Completed")
                                session.update_state("COMPLETED")
                                self.call_logger.save_call(session)
